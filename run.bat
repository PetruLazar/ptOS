...
call ..\qemu\qemu-system-x86_64.exe -monitor stdio -smp 2 -m 1G -cpu Nehalem -device ahci,id=ahci -drive format=raw,file=image.vhd,if=none,id=osimg -device ide-hd,drive=osimg,bus=ahci.0 -drive format=raw,file=bootable/image.vhd,if=none,id=fsimg -device ide-hd,drive=fsimg,bus=ahci.1
