		...
		for (byte deviceNr = 0; deviceNr < 0x20; deviceNr++)
		{
			...
			for (byte funcNr = 0; funcNr < funcLim; funcNr++)
				if (deviceExists(busNr, deviceNr, funcNr))
				{
					...
					if (func.classCode == (byte)DeviceClass::massStorageController && func.subclass == (byte)MassStorageController::IDEController)
						Disk::Initialize(configSpace[4], configSpace[5], configSpace[6], configSpace[7], configSpace[8], configSpace[9]);
